...
import { lokFoodsContent } from './data/lokFoodsContent';
import logoHipervinculo from '@/assets/logo-hipervinculo.png';
import { useToast } from '@/hooks/use-toast';
import { pdf } from '@react-pdf/renderer';
import { LokFoodsPDFDocument } from './pdf/LokFoodsPDFDocument';
...
export function LokFoodsProposal() {
  const documentRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();
  const [isExporting, setIsExporting] = useState(false);
  const content = lokFoodsContent;
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob = await pdf(<LokFoodsPDFDocument logoBase64={logoBase64} />).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Propuesta-Lok-Foods-USA-Amazon.pdf';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      toast({ title: 'PDF Descargado!', description: 'La propuesta ha sido guardada.' });
    } catch (error) {
      console.error('PDF export error:', error);
      toast({ title: 'Error', description: 'Error generando el PDF.', variant: 'destructive' });
    } finally {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b sticky top-0 z-10">
        <h2 className="text-sm sm:text-lg font-bold" style={{ color: '#2d4a2d' }}>
          Propuesta — Lok Foods USA (Amazon)
        </h2>
        <Button
        ...
        <div ref={documentRef}>
          ...
          <Page>
            <div
              ...
              style={{ width: `${PAGE_WIDTH}px`, minHeight: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}
            >
          ...
          <Page>
            ...
            >
              ...
              <p className="text-xs font-bold uppercase tracking-[0.2em] mb-2" style={{ color: '#8BC34A' }}>
                Sobre Nosotros
              </p>
              <h2 className="text-3xl font-extrabold mb-2" style={{ color: '#2d4a2d' }}>
                {content.about.title}
              </h2>
              <p className="font-medium mb-5" style={{ color: '#8BC34A' }}>
              ...
              <div className="grid grid-cols-3 gap-6 mb-10">
                {content.about.stats.map((stat, i) => (
                  <div key={i} className="text-center py-6 rounded-2xl" style={{ backgroundColor: '#f8f9f5' }}>
                    <div className="text-4xl font-extrabold mb-1" style={{ color: '#8BC34A' }}>
                      {stat.value}
                    </div>
              ...
              <div className="flex flex-wrap gap-3">
                {content.about.credentials.map((cred, i) => (
                  <span
                    ...
                    style={{ backgroundColor: 'rgba(139,195,74,0.1)', color: '#2d4a2d' }}
                  >
                    {cred}
                  </span>
                ))}
          ...
          <Page>
            <div
              className="px-16 py-14 flex flex-col"
              style={{ width: `${PAGE_WIDTH}px`, minHeight: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}
            >
              <div className="w-10 h-1 rounded-full mb-8" style={{ backgroundColor: '#8BC34A' }} />
              <p className="text-xs font-bold uppercase tracking-[0.2em] mb-2" style={{ color: '#8BC34A' }}>
              ...
              <h2 className="text-3xl font-extrabold mb-1" style={{ color: '#2d4a2d' }}>
                {content.clientOverview.title}
              </h2>
              <p className="font-medium mb-5" style={{ color: '#8BC34A' }}>
                {content.clientOverview.headline}
              </p>
              <p className="text-gray-600 leading-relaxed mb-6 text-[15px]">
                {content.clientOverview.description}
              </p>
                ...
                {content.clientOverview.services.map((item, i) => (
                  <div key={i} className="p-4 rounded-xl" style={{ backgroundColor: '#f8f9f5' }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="font-bold text-[14px]" style={{ color: '#2d4a2d' }}>
                        {item.title}
                      </h3>
                    </div>
          ...
          <Page>
            ...
            >
              ...
              >
                <h3 className="font-bold text-[14px] mb-1" style={{ color: '#2d4a2d' }}>
                  {content.objective.exclusions.title}
                </h3>
          ...
          <Page>
            ...
            >
              ...
              <h2 className="text-3xl font-extrabold mb-1" style={{ color: '#2d4a2d' }}>
                {content.platformAccess.title}
              </h2>
              <p className="font-medium mb-5" style={{ color: '#8BC34A' }}>
                {content.platformAccess.headline}
              </p>
              <p className="text-gray-600 leading-relaxed mb-8 text-[15px]">
                {content.platformAccess.description}
              </p>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span
                          ...
                          style={{ backgroundColor: 'rgba(139,195,74,0.15)', color: '#2d4a2d' }}
                        >
                          {platform.role}
                        </span>
                      </div>
          ...
          <Page>
            ...
            >
              ...
              <h2 className="text-3xl font-extrabold mb-1" style={{ color: '#2d4a2d' }}>
                {content.amazonAdvertising.title}
              </h2>
              <p className="font-medium mb-5" style={{ color: '#8BC34A' }}>
                {content.amazonAdvertising.headline}
              </p>
              <div
                className="rounded-2xl px-8 py-5 mb-6 flex items-center justify-between"
          ...
          <Page>
            ...
            >
              ...
              <h2 className="text-3xl font-extrabold mb-1" style={{ color: '#2d4a2d' }}>
                {content.listingAdvisory.title}
              </h2>
              <p className="font-medium mb-5" style={{ color: '#8BC34A' }}>
                {content.listingAdvisory.headline}
              </p>
              <div
                className="rounded-2xl px-8 py-5 mb-6 flex items-center justify-between"
                style={{ backgroundColor: '#2d4a2d' }}
              >
                <div>
                  <div className="text-white/50 text-xs font-bold uppercase tracking-widest">Incluido en el Fee</div>
                  <div className="text-white/40 text-xs mt-1">Asesoría en listados y Brand Store</div>
                </div>
                <Store className="w-8 h-8 text-white/70" />
              </div>
              <p className="text-gray-600 leading-relaxed mb-5 text-[15px]">{content.listingAdvisory.description}</p>
              <div className="space-y-2.5 flex-1">
                {content.listingAdvisory.includes.map((item, i) => (
                  <div key={i} className="flex gap-3 p-3 rounded-xl" style={{ backgroundColor: '#f8f9f5' }}>
                    <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: '#8BC34A' }} />
                    <div>
                      <h3 className="font-bold text-[14px]" style={{ color: '#2d4a2d' }}>
          ...
          <Page bg="#f8f9f5">
            <div
              className="px-16 py-12 flex flex-col"
              style={{ width: `${PAGE_WIDTH}px`, minHeight: `${PAGE_HEIGHT}px`, backgroundColor: '#f8f9f5' }}
            >
              ...
              <p className="font-medium mb-6" style={{ color: '#8BC34A' }}>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <h3 className="font-bold text-[16px] mb-6" style={{ color: '#2d4a2d' }}>
                    {content.investment.ongoing.title}
                  </h3>
                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                    <div>
                      <p className="text-[14px] text-gray-700 font-medium">
                        {content.investment.ongoing.commission.label}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5">
                        {content.investment.ongoing.commission.basis}
                      </p>
                    </div>
                    <span className="text-5xl font-extrabold" style={{ color: '#8BC34A' }}>
                      {content.investment.ongoing.commission.rate}
                    </span>
                  </div>
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    {content.investment.ongoing.commission.description}
                  </p>
                </div>
                ...
                <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <h3 className="font-bold text-[16px] mb-6" style={{ color: '#2d4a2d' }}>
                    {content.investment.ongoing.clientPays.title}
                  </h3>
                  <div className="space-y-4">
                    {content.investment.ongoing.clientPays.items.map((item, i) => (
                      <div key={i} className="flex gap-3 items-start">
                        <TrendingUp className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#8BC34A' }} />
                        <div>
              ...
              <div className="bg-white rounded-2xl p-6 mb-5" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="font-bold text-[15px]" style={{ color: '#2d4a2d' }}>
                    {content.investment.example.title}
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-[1fr_auto] gap-x-6 gap-y-1.5">
                  {content.investment.example.rows.map((row, i) => (
                    <div key={i} className="contents">
                      ...
                      <span className="text-[13px] font-semibold text-right" style={{ color: '#2d4a2d' }}>
                        {row.value}
                      </span>
                    </div>
                  ))}
                  <div className="col-span-2 border-t border-gray-200 my-1" />
                  <span className="text-[14px] font-bold" style={{ color: '#2d4a2d' }}>
                    {content.investment.example.netSales.label}
                  </span>
                  <span className="text-xl font-extrabold text-right" style={{ color: '#8BC34A' }}>
                    {content.investment.example.netSales.value}
                  </span>
                  <span className="text-[14px] font-bold" style={{ color: '#2d4a2d' }}>
                    {content.investment.example.commission.label}
                  </span>
                  <span className="text-xl font-extrabold text-right" style={{ color: '#2d4a2d' }}>
                    {content.investment.example.commission.value}
                  </span>
                </div>
                <p className="text-[10px] text-gray-400 mt-3 leading-relaxed">
                  {content.investment.example.note}
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <div className="flex items-center justify-between">
                  <div className="flex-1 pr-6">
                    <h3 className="font-bold text-[15px]" style={{ color: '#2d4a2d' }}>
                      {content.investment.timeline.title}
                    </h3>
                    <p className="text-[12px] text-gray-500 mt-1 leading-relaxed">
                      {content.investment.timeline.description}
                    </p>
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <span className="text-4xl font-extrabold leading-none" style={{ color: '#8BC34A' }}>
                      {content.investment.timeline.duration}
                    </span>
                    <span className="block text-lg font-bold" style={{ color: '#8BC34A' }}>
                      {content.investment.timeline.durationUnit}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <Page>
            ...
            >
              ...
              <p className="font-medium mb-8" style={{ color: '#8BC34A' }}>
                {content.terms.headline}
              </p>
          ...
          <Page bg="#f8f9f5">
            ...
            >
              ...
              <h2 className="text-3xl font-extrabold mb-8" style={{ color: '#2d4a2d' }}>
                {content.legalTerms.title}
              </h2>
              <div className="space-y-5">
                {content.legalTerms.sections.map((section, i) => (
                  <div key={i}>
                    <h3 className="font-extrabold text-[15px] mb-2.5" style={{ color: '#2d4a2d' }}>
                      {section.heading}
                    </h3>
                    <ol className="space-y-2 pl-6 list-decimal">
                      {section.items.map((item, j) => (
                        <li key={j} className="text-[12px] text-gray-600 leading-relaxed pl-1">
                          {item}
                        </li>
                      ))}
                    </ol>
          ...
          <Page>
            ...
            >
              <div className="px-16 py-14 flex-1" style={{ backgroundColor: '#2d4a2d' }}>
                <p className="text-xs font-bold uppercase tracking-[0.2em] mb-3" style={{ color: '#8BC34A' }}>
                  Contáctanos
                </p>
                ...
                <div className="grid grid-cols-2 gap-8 text-white/80">
                  <div className="space-y-6">
                    <div>
                      <p className="text-xs uppercase tracking-[0.15em] mb-1.5" style={{ color: 'rgba(139,195,74,0.5)' }}>
                        Email
                      </p>
