...
const s = StyleSheet.create({
  ...
  pageBg: { padding: 50, backgroundColor: bg, fontFamily: 'Helvetica' },
...
export function TailyPDFDocument({ logoBase64 }: Props) {
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.page}>
        ...
        {/* Exclusions */}
        <View style={{ borderWidth: 1, borderColor: 'rgba(139,195,74,0.3)', borderStyle: 'dashed', borderRadius: 10, padding: 12, marginTop: 8 }}>
          <Text style={{ fontSize: 10, fontWeight: 'bold', color: green, marginBottom: 3 }}>{content.objective.exclusions.title}</Text>
          <Text style={{ fontSize: 8, color: gray, lineHeight: 1.5 }}>{content.objective.exclusions.description}</Text>
        </View>
        <Text style={s.footer}>Hipervínculo · hipervinculo.net</Text>
      </Page>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>CORE SERVICE</Text>
        ...
        <View style={{ backgroundColor: green, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 16, marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={{ fontSize: 9, fontWeight: 'bold', color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>PERFORMANCE-BASED</Text>
            <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>5% of Net Profit Commission</Text>
          </View>
          <Text style={{ fontSize: 28, fontWeight: 'bold', color: 'white' }}>5%</Text>
        </View>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>FINANCIAL TRACKING</Text>
        <Text style={{ fontSize: 22, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.financialTracking.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.financialTracking.headline}</Text>
        <Text style={{ fontSize: 10, color: gray, lineHeight: 1.6, marginBottom: 18 }}>{content.financialTracking.description}</Text>
        {content.financialTracking.features.map((item, i) => (
          <View key={i} style={{ backgroundColor: bg, borderRadius: 10, padding: 14, marginBottom: 8, flexDirection: 'row', gap: 10 }}>
            <View style={{ paddingTop: 1 }}><BarChartIcon size={13} /></View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 10, fontWeight: 'bold', color: green, marginBottom: 3 }}>{item.title}</Text>
              <Text style={{ fontSize: 8.5, color: gray, lineHeight: 1.5 }}>{item.description}</Text>
            </View>
          </View>
        ))}
        <Text style={s.footer}>Hipervínculo · hipervinculo.net</Text>
      ...
      <Page size="A4" style={s.pageBg}>
        ...
        {/* Ongoing Card */}
        <View style={{ backgroundColor: 'white', borderRadius: 14, padding: 22, marginBottom: 16 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, paddingBottom: 12, borderBottomWidth: 0.5, borderBottomColor: '#f0f0f0' }}>
            ...
            <Text style={{ fontSize: 32, fontWeight: 'bold', color: lime, flexShrink: 0 }}>{content.investment.ongoing.commission.rate}</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, paddingBottom: 12, borderBottomWidth: 0.5, borderBottomColor: '#f0f0f0' }}>
            <View style={{ flex: 1, paddingRight: 8 }}>
              <Text style={{ fontSize: 10, color: '#374151', fontWeight: 'medium' }}>{content.investment.ongoing.financialTool.label}</Text>
              <Text style={{ fontSize: 8, color: grayLight, marginTop: 2 }}>{content.investment.ongoing.financialTool.detail}</Text>
            </View>
            <Text style={{ fontSize: 14, fontWeight: 'bold', color: green, flexShrink: 0 }}>{content.investment.ongoing.financialTool.price}</Text>
          </View>
        ...
        {/* Commission Example */}
        <View style={{ backgroundColor: 'white', borderRadius: 14, padding: 16, marginBottom: 14 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 14 }}>
          ...
          <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 14, paddingVertical: 16, alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 22, fontWeight: 'bold', color: lime }}>15–25%</Text>
          </View>
        </View>
      ...
      <Page size="A4" style={s.page}>
        ...
        {content.terms.sections.map((section, i) => {
          ...
          return (
            <View key={i} style={{ backgroundColor: bg, borderRadius: 12, padding: 12, marginBottom: 8, flexDirection: 'row', gap: 12 }}>
      ...
      <Page size="A4" style={s.pageBg} wrap={false}>
        <View style={s.accent} />
