...
import { rumbasEventRentalsProposalContent } from '../data/rumbasEventRentalsProposalContent';
...
export function RumbasEventRentalsPDFDocument({ logoBase64 }: Props) {
  const content = rumbasEventRentalsProposalContent;
  ...
  return (
    <Document>
        ...
        <View style={{ flexDirection: 'row', gap: 14, marginBottom: 24 }}>
          {content.about.stats.map((stat, i) => (
            <View key={i} style={{ flex: 1, backgroundColor: bg, borderRadius: 12, paddingVertical: 16, alignItems: 'center' }}>
              <Text style={{ fontSize: 24, fontWeight: 'bold', color: lime, marginBottom: 4 }}>{stat.value}</Text>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={{ fontSize: 20, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.webService.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.webService.headline}</Text>

        <View style={{ backgroundColor: green, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 14, marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Text style={{ fontSize: 9, fontWeight: 'bold', color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>COMPONENTE DEL PAQUETE</Text>
          <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>{content.webService.priceLabel}</Text>
        </View>
      ...
      {/* PAGE 4: AI Photography */}
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>SERVICIO 2</Text>
        <Text style={{ fontSize: 20, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.leadGenService.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.leadGenService.headline}</Text>

        <View style={{ backgroundColor: green, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 14, marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={{ fontSize: 9, fontWeight: 'bold', color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>COMPONENTE DEL PAQUETE</Text>
            <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.4)', marginTop: 3 }}>{content.leadGenService.mediaSpend} · {content.leadGenService.mediaSpendLabel}</Text>
          </View>
          <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>{content.leadGenService.retainerLabel}</Text>
        </View>

        <Text style={{ fontSize: 10, color: gray, lineHeight: 1.6, marginBottom: 12 }}>{content.leadGenService.description}</Text>
      ...
      <Page size="A4" style={s.pageBg}>
        ...
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 18 }}>{content.investment.headline}</Text>

        {/* Total + Installments */}
        <View style={{ backgroundColor: green, borderRadius: 14, padding: 20, marginBottom: 18 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <Text style={{ fontSize: 9, fontWeight: 'bold', color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>{content.investment.totals.totalLabel.toUpperCase()}</Text>
            <Text style={{ fontSize: 36, fontWeight: 'bold', color: 'white' }}>{content.investment.totals.total}</Text>
          </View>
          <View style={{ borderTopWidth: 0.5, borderTopColor: 'rgba(255,255,255,0.15)', paddingTop: 12, flexDirection: 'row', gap: 14 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 8, color: lime, letterSpacing: 1, marginBottom: 4 }}>{content.investment.totals.installment1Label.toUpperCase()}</Text>
              <Text style={{ fontSize: 22, fontWeight: 'bold', color: 'white' }}>{content.investment.totals.installment1}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 8, color: lime, letterSpacing: 1, marginBottom: 4 }}>{content.investment.totals.installment2Label.toUpperCase()}</Text>
              <Text style={{ fontSize: 22, fontWeight: 'bold', color: 'white' }}>{content.investment.totals.installment2}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 14, marginBottom: 14 }}>
          <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 14, padding: 16 }}>
            <Text style={{ fontSize: 11, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.investment.setup.title}</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: lime, marginBottom: 10 }}>{content.investment.setup.price}</Text>
            {content.investment.setup.includes.map((item, i) => (
              <View key={i} style={{ flexDirection: 'row', gap: 6, marginBottom: 4, alignItems: 'flex-start' }}>
                <View style={{ paddingTop: 1 }}><CheckIcon size={9} /></View>
                <Text style={{ fontSize: 8, color: gray, flex: 1 }}>{item}</Text>
              </View>
            ))}
          </View>
          <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 14, padding: 16 }}>
            <Text style={{ fontSize: 11, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.investment.monthly.title}</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: lime, marginBottom: 10 }}>{content.investment.monthly.price}</Text>
      ...
      <Page size="A4" style={{ fontFamily: 'Helvetica', padding: 0 }}>
        ...
        <View style={{ padding: '16 40', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ width: 80 }}>
            <Image src={logoBase64} style={{ height: 22, objectFit: 'contain', objectPositionX: 'left' }} />
          </View>
          <Text style={{ fontSize: 7, color: grayLight }}>© {new Date().getFullYear()} Hipervinculo LLC</Text>
        </View>
      </Page>
    </Document>
  );
