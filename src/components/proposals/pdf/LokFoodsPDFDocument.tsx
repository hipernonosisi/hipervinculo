...
const StoreIcon = ({ size = 14 }: { size?: number }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24">
    <Path d="M3 3h18v18H3z" fill="none" stroke={lime} strokeWidth="2" />
    <Path d="M3 9h18M9 21V9M15 21V9" fill="none" stroke={lime} strokeWidth="2" strokeLinecap="round" />
  </Svg>
);

const MegaphoneIcon = ({ size = 14 }: { size?: number }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24">
    <Path d="M3 11l18-5v12L3 13v-2z" fill="none" stroke={lime} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <Path d="M11.6 16.8a3 3 0 11-5.8-1.6" fill="none" stroke={lime} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);
...
export function LokFoodsPDFDocument({ logoBase64 }: Props) {
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>CLIENTE</Text>
        <Text style={{ fontSize: 22, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.clientOverview.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.clientOverview.headline}</Text>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
          {content.clientOverview.services.map((item, i) => (
            <View key={i} style={{ width: '48%', backgroundColor: bg, borderRadius: 10, padding: 12, marginBottom: 6 }}>
              <Text style={{ fontSize: 10, fontWeight: 'bold', color: green, marginBottom: 3 }}>{item.title}</Text>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>ALCANCE</Text>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.accent} />
        <Text style={{ fontSize: 9, fontWeight: 'bold', color: lime, letterSpacing: 3, marginBottom: 6 }}>REQUISITOS</Text>
        ...
        {content.platformAccess.platforms.map((platform, i) => (
          <View key={i} style={{ backgroundColor: bg, borderRadius: 10, padding: 14, marginBottom: 8, flexDirection: 'row', gap: 12 }}>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={{ fontSize: 22, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.amazonAdvertising.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.amazonAdvertising.headline}</Text>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={{ fontSize: 22, fontWeight: 'bold', color: green, marginBottom: 4 }}>{content.listingAdvisory.title}</Text>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: lime, marginBottom: 14 }}>{content.listingAdvisory.headline}</Text>
        <View style={{ backgroundColor: green, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 16, marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={{ fontSize: 9, fontWeight: 'bold', color: 'rgba(255,255,255,0.5)', letterSpacing: 2 }}>INCLUIDO EN EL FEE</Text>
            <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>Asesoría en listados y Brand Store</Text>
          </View>
          <StoreIcon size={24} />
        </View>
        <Text style={{ fontSize: 10, color: gray, lineHeight: 1.6, marginBottom: 12 }}>{content.listingAdvisory.description}</Text>
      ...
      <Page size="A4" style={{ ...s.pageBg, padding: 50 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 16, marginBottom: 14 }}>
          ...
          <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 14, padding: 22 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingBottom: 12, borderBottomWidth: 0.5, borderBottomColor: '#f0f0f0' }}>
              ...
              <Text style={{ fontSize: 36, fontWeight: 'bold', color: lime, flexShrink: 0 }}>{content.investment.ongoing.commission.rate}</Text>
            </View>
          ...
          {/* Client Pays */}
          <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 14, padding: 22 }}>
            <Text style={{ fontSize: 12, fontWeight: 'bold', color: green, marginBottom: 16 }}>{content.investment.ongoing.clientPays.title}</Text>
            {content.investment.ongoing.clientPays.items.map((item, i) => (
              <View key={i} style={{ flexDirection: 'row', gap: 10, marginBottom: 12, alignItems: 'flex-start' }}>
                <View style={{ paddingTop: 1 }}><MegaphoneIcon size={12} /></View>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#374151', marginBottom: 1 }}>{item.name}</Text>
                  <Text style={{ fontSize: 7.5, color: grayLight }}>{item.detail}</Text>
                </View>
              </View>
            ))}
          </View>
