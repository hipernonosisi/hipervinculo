...
import { valoresInmobiliariosProposalContent } from './data/valoresInmobiliariosProposalContent';
import logoHipervinculo from '@/assets/logo-hipervinculo.png';
import { useToast } from '@/hooks/use-toast';
import { pdf } from '@react-pdf/renderer';
import { ValoresInmobiliariosPDFDocument } from './pdf/ValoresInmobiliariosPDFDocument';
...
export function ValoresInmobiliariosProposal() {
  const documentRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();
  const [isExporting, setIsExporting] = useState(false);
  const content = valoresInmobiliariosProposalContent;
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob = await pdf(
        <ValoresInmobiliariosPDFDocument logoBase64={logoBase64} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Propuesta-Valores-Inmobiliarios-America.pdf';
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b sticky top-0 z-10">
        <h2 className="text-sm sm:text-lg font-bold" style={{ color: '#2d4a2d' }}>
          Propuesta — Valores Inmobiliarios de America LLC
        </h2>
