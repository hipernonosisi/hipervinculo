...
import { RumbasEventRentalsPDFDocument } from './pdf/RumbasEventRentalsPDFDocument';
...
export function RumbasEventRentalsProposal() {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob = await pdf(
        <RumbasEventRentalsPDFDocument logoBase64={logoBase64} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Propuesta-Rumbas-Event-Rentals.pdf';
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b sticky top-0 z-10">
        <h2 className="text-sm sm:text-lg font-bold" style={{ color: '#2d4a2d' }}>
          Propuesta — Rumbas Event Rentals
        </h2>
        ...
        <div ref={documentRef}>
            ...
            <div className="px-16 py-10 flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}>
              ...
              <div className="rounded-2xl px-8 py-4 mb-4 flex items-center justify-between" style={{ backgroundColor: '#2d4a2d' }}>
                ...
                <div className="text-right">
                  <span className="text-2xl font-extrabold text-white">{content.webService.priceLabel}</span>
                </div>
              </div>
          ...
          <Page>
            <div className="px-16 py-14 flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}>
              ...
              <div className="rounded-2xl px-8 py-5 mb-6 flex items-center justify-between" style={{ backgroundColor: '#2d4a2d' }}>
                <div>
                  <div className="text-white/50 text-xs font-bold uppercase tracking-widest">Componente del Paquete</div>
                  <div className="text-white/40 text-xs mt-1">{content.leadGenService.mediaSpend} · {content.leadGenService.mediaSpendLabel}</div>
                </div>
                <div className="text-right">
                  <span className="text-2xl font-extrabold text-white">{content.leadGenService.retainerLabel}</span>
                </div>
              </div>

              <p className="text-gray-600 leading-relaxed mb-4 text-[14px]">{content.leadGenService.description}</p>
          ...
          <Page bg="#f8f9f5">
            <div className="px-16 py-14 flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#f8f9f5' }}>
              ...
              <p className="font-medium mb-8" style={{ color: '#8BC34A' }}>{content.investment.headline}</p>

              {/* Total + Installments hero */}
              <div className="rounded-2xl p-8 mb-8" style={{ backgroundColor: '#2d4a2d' }}>
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <div className="text-white/50 text-xs font-bold uppercase tracking-widest">{content.investment.totals.totalLabel}</div>
                  </div>
                  <span className="text-5xl font-extrabold text-white">{content.investment.totals.total}</span>
                </div>
                <div className="border-t border-white/10 pt-5 grid grid-cols-2 gap-6">
                  <div>
                    <div className="text-xs uppercase tracking-wider mb-1" style={{ color: '#8BC34A' }}>{content.investment.totals.installment1Label}</div>
                    <div className="text-3xl font-extrabold text-white">{content.investment.totals.installment1}</div>
                  </div>
                  <div>
                    <div className="text-xs uppercase tracking-wider mb-1" style={{ color: '#8BC34A' }}>{content.investment.totals.installment2Label}</div>
                    <div className="text-3xl font-extrabold text-white">{content.investment.totals.installment2}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <h3 className="font-bold text-[15px] mb-2" style={{ color: '#2d4a2d' }}>{content.investment.setup.title}</h3>
                  <div className="text-xl font-extrabold mb-4" style={{ color: '#8BC34A' }}>{content.investment.setup.price}</div>
                  <ul className="space-y-2">
                    {content.investment.setup.includes.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 text-[12px] text-gray-600">
                        <CheckCircle className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" style={{ color: '#8BC34A' }} />
                        {item}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <h3 className="font-bold text-[15px] mb-2" style={{ color: '#2d4a2d' }}>{content.investment.monthly.title}</h3>
                  <div className="text-xl font-extrabold mb-4" style={{ color: '#8BC34A' }}>{content.investment.monthly.price}</div>
                  <ul className="space-y-2">
          ...
          <Page>
            <div className="flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}>
              ...
              <div className="px-16 py-8 flex items-center justify-between bg-white">
                <img src={logoHipervinculo} alt="Hipervinculo" className="h-8" />
                <p className="text-xs text-gray-400">© {new Date().getFullYear()} Hipervinculo LLC</p>
              </div>
            </div>
          </Page>
