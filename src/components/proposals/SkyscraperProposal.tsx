...
import { SkyscraperPDFDocument } from './pdf/SkyscraperPDFDocument';
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob = await pdf(
        <SkyscraperPDFDocument logoBase64={logoBase64} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Proposal-Skyscraper-Construction.pdf';
