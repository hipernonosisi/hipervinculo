...
import { Download, CheckCircle, FileText, RefreshCw, Server, Calendar, DollarSign, Clock, Bot, Star, Layers } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { rasettaInnovationsProposalContent } from './data/rasettaInnovationsProposalContent';
import logoHipervinculo from '@/assets/logo-hipervinculo.png';
import { useToast } from '@/hooks/use-toast';
import { pdf } from '@react-pdf/renderer';
import { RasettaInnovationsPDFDocument } from './pdf/RasettaInnovationsPDFDocument';
...
export function RasettaInnovationsProposal() {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const blob = await pdf(
        <RasettaInnovationsPDFDocument logoBase64={logoBase64} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Propuesta-Rasetta-Innovations.pdf';
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto bg-gray-300 py-8 px-4">
        <div ref={documentRef}>
          ...
          {/* PAGE 3: Meta Ads Service (Etapa 1) */}
          <Page>
            <div className="px-16 py-12 flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}>
              ...
              <div className="flex items-center gap-3 mb-1.5">
                <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: '#8BC34A' }}>Etapa 1</span>
                <Layers className="w-4 h-4" style={{ color: '#8BC34A' }} />
              </div>
          ...
          {/* PAGE 4: Bot Service (Etapa 2) - What it does */}
          <Page>
            <div className="px-16 py-14 flex flex-col" style={{ width: `${PAGE_WIDTH}px`, height: `${PAGE_HEIGHT}px`, backgroundColor: '#ffffff' }}>
              ...
              <div className="flex items-center gap-3 mb-2">
                <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: '#8BC34A' }}>Etapa 2</span>
                <Layers className="w-4 h-4" style={{ color: '#8BC34A' }} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <div className="flex items-center gap-2 mb-2">
                    <Layers className="w-5 h-5" style={{ color: '#8BC34A' }} />
                    <h3 className="font-bold text-lg" style={{ color: '#2d4a2d' }}>{content.investment.phase1.title}</h3>
                  </div>
                  <p className="text-xs text-gray-500 mb-6">{content.investment.phase1.subtitle}</p>
                  ...
                  <div className="border-t pt-4 mt-2">
                    <p className="text-[12px] text-gray-500 leading-relaxed">{content.investment.phase1.mediaNote}</p>
                    <p className="text-[11px] text-gray-400 mt-2 italic">{content.investment.phase1.note}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <div className="flex items-center gap-2 mb-2">
                    <Layers className="w-5 h-5" style={{ color: '#8BC34A' }} />
                    <h3 className="font-bold text-lg" style={{ color: '#2d4a2d' }}>{content.investment.phase2.title}</h3>
                  </div>
                  <p className="text-xs text-gray-500 mb-6">{content.investment.phase2.subtitle}</p>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-gray-600">{content.investment.phase2.setup.name}</span>
                    <span className="font-bold text-lg" style={{ color: '#2d4a2d' }}>{content.investment.phase2.setup.price}</span>
                  </div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-gray-600">{content.investment.phase2.monthly.name}</span>
                    <span className="font-bold" style={{ color: '#2d4a2d' }}>{content.investment.phase2.monthly.price}</span>
                  </div>
                  <div className="border-t pt-4 mt-2">
                    <p className="text-[11px] text-gray-400 italic">{content.investment.phase2.note}</p>
                  </div>
                </div>
              </div>
