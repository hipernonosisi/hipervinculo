...
import type { HesacoreReport } from '../data/hesacoreReportData';
...
interface Props {
  report: HesacoreReport;
  logoBase64: string;
}

export function HesacorePDFDocument({ report, logoBase64 }: Props) {
  const colWidths = { month: '16%', spend: '16%', purchases: '12%', revenue: '18%', roas: '12%', cpa: '12%', leads: '14%' };
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={[s.subtitle, { marginBottom: 20 }]}>{report.clientName} · {report.reportPeriod}</Text>
        ...
        <View style={s.kpiRow}>
          ...
          <View style={s.kpiCard}>
            ...
            <Text style={s.kpiValue}>{report.yearTotals.totalPurchases.toLocaleString()}</Text>
          </View>
        </View>
        ...
        <View style={s.tableHeader}>
          ...
          <Text style={[s.thCell, { width: colWidths.leads, textAlign: 'right' }]}>Leads</Text>
        </View>

        {report.monthlyData.map((m, i) => (
          <View key={i} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
            <Text style={[s.tdBold, { width: colWidths.month }]}>{m.monthShort}</Text>
            <Text style={[s.tdCell, { width: colWidths.spend, textAlign: 'right' }]}>${fmt(m.spend)}</Text>
            <Text style={[s.tdCell, { width: colWidths.purchases, textAlign: 'right' }]}>{m.purchases}</Text>
            <Text style={[s.tdCell, { width: colWidths.revenue, textAlign: 'right' }]}>${fmt(m.revenue)}</Text>
            <View style={{ width: colWidths.roas, alignItems: 'flex-end' }}><RoasBadge value={m.roas} /></View>
            <Text style={[s.tdCell, { width: colWidths.cpa, textAlign: 'right' }]}>${fmt(m.cpa)}</Text>
            <Text style={[s.tdCell, { width: colWidths.leads, textAlign: 'right' }]}>{m.leads}</Text>
          </View>
        ))}
        ...
        <View style={s.tableTotalRow}>
          ...
          <Text style={[s.tdBold, { width: colWidths.leads, textAlign: 'right' }]}>{report.yearTotals.totalLeads}</Text>
        </View>

        <Text style={s.footer}>Reporte generado por Hipervínculo · hipervinculo.net · info@hipervinculo.net</Text>
      </Page>

      {/* Page 2: Integrations Status */}
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          <Image src={logoBase64} style={s.logo} />
          <View style={s.accent} />
      ...
      <Page size="A4" style={{ fontFamily: 'Helvetica', padding: 0 }}>
        <View style={{ backgroundColor: green, padding: 40, flex: 1, justifyContent: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 18 }}>
            ...
            <View>
              <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.5)' }}>Teléfono</Text>
              <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold' }}>+1 (786) 505-2484</Text>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <View>
              <Text style={{ fontSize: 8, color: 'rgba(255,255,255,0.5)' }}>Ubicación</Text>
              <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold' }}>Miami, FL</Text>
            </View>
          </View>
        </View>

        <View style={{ backgroundColor: 'white', padding: 40, alignItems: 'center', justifyContent: 'center', height: 200 }}>
          <Image src={logoBase64} style={{ height: 40, marginBottom: 12 }} />
          <Text style={{ fontSize: 10, color: '#888' }}>Marketing Digital · Miami, FL</Text>
        </View>
      </Page>
    </Document>
  );
