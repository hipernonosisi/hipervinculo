...
import { brandIdentityPresentationContent, BrandIdentityLanguage } from './data/brandIdentityPresentationContent';
...
import { BrandIdentityPDFDocument } from './pdf/BrandIdentityPDFSlides';
...
export function BrandIdentityPresentation() {
  const [language, setLanguage] = useState<BrandIdentityLanguage>('en');
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isExporting, setIsExporting] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();
  
  const content = brandIdentityPresentationContent[language];
  ...
  const exportToPDF = useCallback(async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `hipervinculo-brand-identity-${language}.pdf`;
      ...
      toast({
        ...
        description: `hipervinculo-brand-identity-${language}.pdf`,
      });
    } catch (error) {
      ...
      <div className="flex items-center justify-between px-3 py-2 sm:p-4 bg-white border-b gap-2">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <h2 className="text-sm sm:text-lg font-bold truncate" style={{ color: '#2d4a2d' }}>
            {language === 'en' ? 'Brand Identity' : 'Imagen de Marca'}
          </h2>
