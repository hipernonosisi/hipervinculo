...
export function ReportingSlide({ content }: ReportingSlideProps) {
  return (
    <div className="relative w-full h-full flex flex-col p-3 sm:p-6 md:p-8 overflow-hidden" style={{ backgroundColor: '#f8f9f5' }}>
      ...
      {/* Metrics grid */}
      <div className="grid grid-cols-3 gap-0.5 sm:gap-2 md:gap-3 mb-1 sm:mb-3 md:mb-4 flex-1">
        {content.metrics.map((metric, index) => (
          <div 
            key={index}
            className="p-1 sm:p-3 md:p-4 rounded sm:rounded-lg bg-white"
            style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}
          >
            <div className="flex items-center gap-0.5 sm:gap-2 mb-0.5 sm:mb-1">
              <BarChart3 className="w-2 h-2 sm:w-4 sm:h-4 md:w-5 md:h-5" style={{ color: '#8BC34A' }} />
              <h4 className="font-bold text-[6px] sm:text-sm md:text-lg leading-tight" style={{ color: '#2d4a2d' }}>
                {metric.name}
              </h4>
            </div>
      ...
      {/* Deliverables */}
      <div 
        className="p-1.5 sm:p-4 md:p-5 rounded sm:rounded-xl"
        style={{ backgroundColor: '#2d4a2d' }}
      >
        <h3 className="text-[7px] sm:text-base md:text-xl font-bold text-white mb-0.5 sm:mb-2 md:mb-3">Deliverables</h3>
        <div className="grid grid-cols-2 gap-0.5 sm:gap-2 md:gap-3">
          {content.deliverables.map((item, index) => (
            <div key={index} className="flex items-center gap-0.5 sm:gap-2">
              <CheckCircle className="w-2 h-2 sm:w-4 sm:h-4 md:w-5 md:h-5 flex-shrink-0" style={{ color: '#8BC34A' }} />
              <span className="text-white/90 text-[5px] sm:text-xs md:text-base leading-tight">{item}</span>
            </div>
          ))}
        </div>
      </div>
